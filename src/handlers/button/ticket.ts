...
export default {
  ...
  async execute(interaction: ButtonInteraction) {
    ...
    try {
      ...
      await interaction.followUp({
        embeds: [
          createErrorEmbed("ボタンの情報を正しく読み取れませんでした。"),
        ],
        ephemeral: true,
      });
      return;
    ...
    if (!category || category.type !== ChannelType.GuildCategory) {
      await interaction.followUp({
        embeds: [
          createErrorEmbed(
            "指定されたカテゴリーが見つからないか、カテゴリーではありません。",
          ),
        ],
        ephemeral: true,
      });
    ...
    const existingTicket = guild.channels.cache.find(
      (channel) =>
        channel.parentId === category.id && channel.name === ticketChannelName,
    );
    ...
    if (existingTicket) {
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `すでにチケットチャンネル ${existingTicket} があります。`,
        )
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `チケットチャンネル ${ticketChannel} が作成されました。`,
        )
